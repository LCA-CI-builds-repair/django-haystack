...
from haysta        stowed_verbose_name_plural = MockModel._meta.verbo        sf = F            {"q": "test"}, selected_facets=['author:"daniel"'], searchqueryset=self.sqs
        )
        sqs = sf.search()
        ...
        sqs = sf.search()
        self.assertEqual(
            sqs.query.narrow_queries, set(['author:"daniel"', 'author:"chris"'])orm(
        ...
        sqs = sf.search()
        self.assertEqual(sqs.query.narrow_queries, set())al
...
class FacetedSearchFormTestCase(TestCase):
    ...
    def test_init_with_selected_facets(self):
        ...
        self.assertEqual(sf.errors, {})
        self.assertEqual(sf.is_valid(), True)
        self.assertEqual(sf.selected_facets, ["author:daniel", "author:chris"])Form, model_choices
